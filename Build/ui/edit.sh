...
if [[ "$page" == "edit" ]]; then
	...
	synotr_form_select "CutEditorQueue" "CutEditor-Warteliste" \
		'Welche Dateien der CutEditor anbietet (nur die Warteliste). Private Cutlists nicht zu cutlist.at hochladen. Standard: keine lokale und keine bekannte Online-Cutlist.' \
		"$_ce_q_opts"

	synotr_form_switch "CutEditorOtrkeyMp4" "otrkey-AVI im CutEditor als MP4" "${CutEditorOtrkeyMp4:-off}" \
		'ein =&gt; AVI ohne Cutlist können für den Eigengebrauch nach MP4 remuxt und im CutEditor geschnitten werden (OTRavi2mp4, MP4 im Dekodierordner, AVI in den Papierkorb). Diese Cutlists sind privat und nicht für cutlist.at gedacht.<br>aus =&gt; nur otr2-MP4.<br><br>Kein Stammtausch AVI↔MP4 in der Datenbank. Packed-Bitstream kann zittern.'
